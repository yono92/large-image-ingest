...
Checksum and persistent source-identity preparation use the packaged ESM Worker executor from `large-image-ingest/browser`. The example explicitly chooses bounded inline fallback for browsers that cannot start the Worker; cancellation and source replacement still invalidate late results. Recovery recomputes one whole-file identity before enabling Resume, and the UI never renders that digest or a full resume record.
